// IBrick.cpp: implementation of the CIBrick class.
//
//////////////////////////////////////////////////////////////////////

#include "IBrick.h"

//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

int CIBrick::checkCollision(CBin* bin)
{
	int width,height,orientation,posX,posY;
	unsigned char** image;
	width=bin->getWidth();
	height=bin->getHeight();
	image=new unsigned char* [height];
	for(int i=0;i<height;i++)
		image[i]=new unsigned char [width];
	bin->getImage(image);
	orientation=getOrientation();
	posX=getPosX();
	if(posX>40)
		posX-=41;
	posY=getPosY();
	//¡ö
	//¡õ
	//¡ö
	//¡ö
	if(0==orientation)//I-brick×´Ì¬0
	{//×©¿éÅöµ½±ß½ç
		if((posX<0)||(posX>width-1)||
			(posY<1)||(posY+2>height-1))
			return 0;
		//×©¿éÅöµ½ÆäËû¹Ì¶¨×©¿é
		if((image[posY-1][posX]!=0)||
			(image[posY][posX]!=0)||
			(image[posY+1][posX]!=0)||
			(image[posY+2][posX]!=0))
			return 0;
	}
	//¡ö¡ö¡õ¡ö
	if(1==orientation)//I-brick×´Ì¬1
	{//×©¿éÅöµ½±ß½ç
		if((posX<2)||(posX+1>width-1)||
			(posY<0)||(posY>height-1))
			return 0;
		//×©¿éÅöµ½ÆäËû¹Ì¶¨×©¿é
		if((image[posY][posX-2]!=0)||
			(image[posY][posX-1]!=0)||
			(image[posY][posX]!=0)||
			(image[posY][posX+1]!=0))
			return 0;
	}
	//¡ö
	//¡ö
	//¡õ
	//¡ö
	if(2==orientation)//I-brick×´Ì¬2
	{//×©¿éÅöµ½±ß½ç
		if((posX<0)||(posX>width-1)||
			(posY-2<0)||(posY+1>height-1))
			return 0;
		//×©¿éÅöµ½ÆäËû¹Ì¶¨×©¿é
		if((image[posY+1][posX]!=0)||
			(image[posY][posX]!=0)||
			(image[posY-1][posX]!=0)||
			(image[posY-2][posX]!=0))
			return 0;
	}
	//¡ö¡õ¡ö¡ö
	if(3==orientation)//I-brick×´Ì¬3
	{//×©¿éÅöµ½±ß½ç
		if((posX-1<0)||(posX+2>width-1)||
			(posY<0)||(posY>height-1))
			return 0;
		//×©¿éÅöµ½ÆäËû¹Ì¶¨×©¿é
		if((image[posY][posX-1]!=0)||
			(image[posY][posX]!=0)||
			(image[posY][posX+1]!=0)||
			(image[posY][posX+2]!=0))
			return 0;
	}
	return 1;
}
void CIBrick::operator>>(unsigned char** binImage)
{
	int orientation,posX,posY;
	unsigned char colour;
	posX=getPosX();
	if(posX>40)
		posX-=41;
	posY=getPosY();
	orientation=getOrientation();
	colour=getColour();
		//¡ö
		//¡õ
		//¡ö
		//¡ö
		if(0==orientation)//I-brick×´Ì¬0
		{
			binImage[posY-1][posX]=colour;
			binImage[posY][posX]=colour;
			binImage[posY+1][posX]=colour;
			binImage[posY+2][posX]=colour;
		}
		//¡ö¡ö¡õ¡ö
		if(1==orientation)//I-brick×´Ì¬1
		{
			binImage[posY][posX-2]=colour;
			binImage[posY][posX-1]=colour;
			binImage[posY][posX]=colour;
			binImage[posY][posX+1]=colour;
		}
		//¡ö
		//¡ö
		//¡õ
		//¡ö
		if(2==orientation)//I-brick×´Ì¬2
		{
			binImage[posY-2][posX]=colour;
			binImage[posY-1][posX]=colour;
			binImage[posY][posX]=colour;
			binImage[posY+1][posX]=colour;
		}
		//¡ö¡õ¡ö¡ö
		if(3==orientation)//I-brick×´Ì¬3
		{
			binImage[posY][posX-1]=colour;
			binImage[posY][posX]=colour;
			binImage[posY][posX+1]=colour;
			binImage[posY][posX+2]=colour;
		}
}
void CIBrick::putAtTop(int newOrient,int newPosX)
{
	setPosX(newPosX);
	setOrientation(newOrient);
	switch(newOrient)
	{
	case 0:
		setPosY(1);
		break;
	case 1:
		setPosY(0);
		break;
	case 2:
		setPosY(2);
		break;
	case 3:
		setPosY(0);
		break;
	}
}

